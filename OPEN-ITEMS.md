...
| OI-09 | **"Wellness Space with City View"** source image has a Wix scroll-cue disc baked into the pixels (x 0.59–0.68, y 0.17–0.25 of the 2400² master). | Original file | The portfolio tile and the featured-work card are cropped so it does not show; the case-study hero, the full-frame figure and the OG image still contain it. **A clean export would resolve all of them** — the master is deliberately not retouched, since it is the client's own photograph. |
...
| OI-43 | **About page headline** now reads "Spaces that balance ease and elegance." (a rephrasing of the brief's positioning) rather than "A woman-owned, New York–based interior design firm." The woman-owned / New York facts still render verbatim in the paragraph directly beneath. | `src/components/sections/About` | Please confirm the headline preference. |
| OI-44 | **Case-study depth.** Aromatherapy and Natural Elements and Wellness Space with City View have a single photograph each, so those pages are hero + description + that one image. | Scrape | More imagery (or a note that these are single-image collections) would let the layout do more. |
| OI-45 | **Materials swatches** are composed room frames rather than macro material details, because the scraped set contains no close-up material photography. | `content/overrides.json` → `home.materials` | Four close-ups (wood, stone, textile, brass) would make this section sing. |
| OI-46 | **`/services` jump-links** sit below the fold at 1024. Closing the gap means stepping the display headline down at `lg` — a design choice, not a defect. | `src/components/sections/Services` | Say the word and it is a one-line change. |
| OI-47 | **Disciplines blurbs between 1024–1280** sit behind the same expand toggles used on touch, because four columns at that width give a 6–11 character measure. | `src/components/sections/Disciplines` | Deliberate trade; a three-up layout at that band is the alternative. |
...
| OI-48 | `content/overrides.json` has no `focalPoint` key, so per-image crop art direction cannot yet survive `pnpm content:images`. | Add the key to the merge in `scripts/optimize-images.ts` when per-image cropping is wanted. |
| OI-49 | The footer's firm descriptor is derived in `Footer.tsx` from `SITE.positioning`; an explicit `descriptor` field in `src/lib/site.ts` would be clearer. | Cosmetic. |
